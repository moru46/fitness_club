...
PRIMARY KEY (`Esercizio`,`Giorno`,`SchedaAllenamento`) 
) ENGINE=InnoDB DEFAULT CHARSET=latin1; 
...
elseif not exists (select *
					from turnazione t inner join istruttorecorsi ic on ic.Istruttore = t.Dipendente
                    where ic.istruttore in (select ic2.istruttore
												from istruttorecorsi ic2 
												where ic2.corso = new.codcorso)
					and t.giorno = new.giorno)
...
THEN 	SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Giorno libero istruttore';
...
elseif not exists ( select *
                    ...
                    where ic.istruttore in (select ic2.istruttore
												...
												where ic2.corso = new.codcorso)
					and t.giorno = new.giorno
                    and new.orainizio >= t.inizioturno
                    and new.orafine <= t.fineturno)
                    
THEN 	SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Orario istruttore incompatibile';
...
DROP TRIGGER IF EXISTS AggiortaTotAttrezzi;
DELIMITER $$
...
DROP TRIGGER IF EXISTS ApportoCalorico;
DELIMITER $$
...
ADD FOREIGN KEY(`Giorno`) REFERENCES `CalendarioDieta`(`Giorno`) 
ON DELETE NO ACTION ON UPDATE NO ACTION;*/
...
ON DELETE NO ACTION ON UPDATE NO ACTION,/*
ADD FOREIGN KEY(`Giorno`) REFERENCES `Configurazione`(`Giorno`) 
ON DELETE NO ACTION ON UPDATE NO ACTION,*/
...
ON DELETE CASCADE ON UPDATE NO ACTION,/*
ADD FOREIGN KEY(`Giorno`) REFERENCES `Monitoraggio`(`Giorno`) 
ON DELETE NO ACTION ON UPDATE NO ACTION,*/
ADD FOREIGN KEY(`Macchinario`) REFERENCES `Apparecchiature`(`CodAttrezzo`) 
ON DELETE CASCADE ON UPDATE NO ACTION; 
...
ON DELETE CASCADE ON UPDATE CASCADE, /*
ADD FOREIGN KEY(`Giorno`) REFERENCES `CalendarioDietaC`(`Giorno`) 
ON DELETE RESTRICT ON UPDATE CASCADE, */
...
('0001','Via di Danzica 29, Livorno','200','3312969124','450'), -- Livorno
...
INSERT INTO SaleAllenamento VALUES  -- IMPOSTARE TOT.ATTREZZI CON UNA UPDATE (Operazioni)
...
/* ROTAZIONE MAGAZINO */
/*
-- CONTROLLO SCADENZE DATO IL CENTRO
...
-- VENDITA SCONTATA DATA LA SCADENZA
...
-- EVENT PER LA GESTIONE DELLE PROMOZIONI ONGI SETTIMANA
